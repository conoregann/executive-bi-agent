...
- Start one branch from `main` for a distinct capability. Keep working on that
  branch across sessions until the capability is complete; do not create a new
  branch for a prompt, task list item, or commit.
- Use `feat/<short-name>`, `fix/<short-name>`, or `chore/<short-name>`.
- Make coherent Conventional Commits as useful recovery and review points
  within the branch. Commits should be understandable and safe to review, but
  the merge-ready capability—not a collection of manually managed chunks—is
  the delivery unit.
...
3. Implement the complete capability, including representative tests,
   meaningful failure cases, and evaluation where required. Automate
   repeatable validation rather than relying on manual coordination.
...
2. Implement the complete branch capability.
...
5. Remove stale code or documentation, then commit the reviewable capability.
   Keep the branch open if the defined capability still has required work
   remaining.
